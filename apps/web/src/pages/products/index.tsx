...
import { Table, Form, Input, Select, Space, Tag, Descriptions, InputNumber, Divider, Switch } from "antd";

const PRICE_TYPES = [
  { label: "Harga Produksi", value: "production" },
  { label: "Harga Agen", value: "agent" },
  { label: "Harga Sales", value: "sales" },
];

const priceLabel = (type: string) =>
  type === "production" ? "Harga Produksi" : type === "agent" ? "Harga Agen" : "Harga Sales";

const PriceFields: React.FC = () => (
  <Form.List
    name="prices"
    initialValue={[
      { price_type: "production", price: 0 },
      { price_type: "agent", price: 0 },
      { price_type: "sales", price: 0 },
    ]}
  >
    {(fields) =>
      fields.map((field) => (
        <Space key={field.key} style={{ display: "flex", marginBottom: 8 }} align="baseline">
          <Form.Item {...field} name={[field.name, "price_type"]} label="Tipe">
            <Select style={{ width: 180 }} disabled options={PRICE_TYPES} />
          </Form.Item>
          <Form.Item {...field} name={[field.name, "price"]} label="Harga (Rp)" rules={[{ required: true }]}>
            <InputNumber<number>
              ...
              parser={(v) => Number(v ? String(v).replace(/\./g, "") : 0)}
            />
          </Form.Item>
        </Space>
      ))
...
export const ProductCreate: React.FC = () => {
  ...
  const { selectProps: producerSelectProps } = useSelect({
    resource: "producers",
    optionLabel: "name",
    optionValue: "id",
  ...
  return (
    <Create saveButtonProps={saveButtonProps} title="Tambah Produk">
      <Form {...formProps} layout="vertical">
        ...
        <Form.Item label="Produsen" name="producer_id">
          <Select {...producerSelectProps} showSearch optionFilterProp="label" allowClear placeholder="Pilih produsen (opsional)" />
        </Form.Item>
  ...
  return (
    <Edit saveButtonProps={saveButtonProps} title="Edit Produk">
      <Form {...formProps} layout="vertical">
        ...
        <Form.Item label="Deskripsi" name="description"><Input.TextArea rows={3} /></Form.Item>
        <Form.Item label="Aktif" name="is_active" valuePropName="checked"><Switch /></Form.Item>
        <Divider>Harga</Divider>
        <PriceFields />
      </Form>
    </Edit>
  ...
  return (
    <Show title="Detail Produk">
      <Descriptions bordered column={1} size="small">
        ...
        <Descriptions.Item label="Produsen">{record?.producer?.name || "-"}</Descriptions.Item>
      ...
      {record?.prices && (
        <Descriptions bordered column={1} size="small" title="Harga" style={{ marginTop: 16 }}>
          {record.prices.map((p: any) => (
            <Descriptions.Item key={p.price_type} label={priceLabel(p.price_type)}>
