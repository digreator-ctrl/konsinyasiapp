...
import { Table, Form, Input, Select, InputNumber, Space, Tag, Descriptions } from "antd";
  ...
  return (
    <List title="Penerimaan Stok (Stok Masuk)">
      <Table {...tableProps} rowKey="id" size="middle">
        ...
        <Table.Column dataIndex="producer_name" title="Produsen" render={(v) => v || "-"} />
  ...
  return (
    <Create saveButtonProps={saveButtonProps} title="Catat Stok Masuk">
      <Form {...formProps} layout="vertical">
        ...
        <Form.Item label="Produk" name="product_id" rules={[{ required: true }]}>
          <Select {...productSelectProps} showSearch optionFilterProp="label" placeholder="Pilih produk" />
        </Form.Item>
        ...
        <Form.Item label="Harga Produksi (Rp)" name="production_cost" rules={[{ required: true }]}>
          <InputNumber<number>
            min={0}
            style={{ width: "100%" }}
  ...
  return (
    <Show title="Detail Stok Masuk">
      <Descriptions bordered column={1} size="small">
        <Descriptions.Item label="Produk">{record?.product?.name || record?.product_id}</Descriptions.Item>
